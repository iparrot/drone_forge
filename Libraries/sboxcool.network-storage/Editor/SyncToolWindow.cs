...
// ── Revision change watcher ──
	...
	private async Task<(bool epOk, bool colOk, bool wfOk)> DoPushAllBatched()
	{
		try
		{
			...
			// The production batch /sync route stages endpoints correctly for publishTarget=next,
			// but historically did not stage collections. Do not route endpoints through the
			// dedicated endpoint PUT here: that route can report success while leaving only
			// the live endpoint unchanged, which then fails post-push verification against
			// the selected next target. Split the next-release push: batch endpoints/workflows,
			// dedicated collections.
			if ( hasCollections && string.Equals( _publishTarget, "next", StringComparison.OrdinalIgnoreCase ) )
				return await DoPushNextWithDedicatedCollections( localEps, localCols, localWfs, hasEndpoints, hasCollections, hasWorkflows );
	...
	/// <summary>
	/// Next-release push path when collections are present: /sync stages endpoints,
	/// while the dedicated collection route stages collection overrides.
	/// </summary>
	private async Task<(bool epOk, bool colOk, bool wfOk)> DoPushNextWithDedicatedCollections( List<JsonElement> localEps, List<JsonElement> localCols, List<JsonElement> localWfs,
		bool hasEndpoints, bool hasCollections, bool hasWorkflows )
	{
		var endpointsAndWorkflowsTask = (hasEndpoints || hasWorkflows)
			? DoPushEndpointsAndWorkflowsBatched( localEps, localWfs, hasEndpoints, hasWorkflows )
			: Task.FromResult( (epOk: false, wfOk: false) );
		var collectionsTask = hasCollections ? DoPushCollections() : Task.FromResult( false );

		await Task.WhenAll( endpointsAndWorkflowsTask, collectionsTask );

		var (epOk, wfOk) = await endpointsAndWorkflowsTask;
		return (epOk, await collectionsTask, wfOk);
	}
